<script lang="ts">
	import { Button, Dialog, Overlay, ProgressCircle, TextField, Tooltip } from 'svelte-ux';
	...
	let isLoading = false;
	let isOpenDescriptionModal = false;
	let tempColumn = {};
	let tempDescription = '';
	...
	const toggleDescriptionModal = () => isOpenDescriptionModal = !isOpenDescriptionModal;

	function getColumn(subjectId: string, tableId: string, physicalName: string) {
		let subject = erd.subjects.find((s: { id: string }) => s.id === subjectId);
		let table = subject?.tables.find((t) => t.id === tableId);

		return table?.columns.find((c) => c.physicalName === physicalName);
	}

	const openModal = (subjectId: string, tableId: string, physicalName: string) => {
		let column = getColumn(subjectId, tableId, physicalName);
		tempColumn = column;
		tempDescription = column.description;
		isOpenDescriptionModal = true;
	}
	const bindValue = () => {
		tempColumn.description = tempDescription;
		erd = erd;
	...
	<div class="mt-1">
		...
		<table class="w-full">
			...
			<tr>
				...
				<th rowspan="2">기본값</th>
				<th rowspan="2" class="min-w-28">비고</th>
			</tr>
			<tr>
				<th>논리</th>
				<th>물리</th>
				...
				{#each subject.tables as table, tableIndex}
					{#each table.columns as column, columnIndex}
						<tr class="{tableIndex === 0 && columnIndex === 0 ? 'split' : ''}">
							...
							<td on:dblclick={() => openModal(subject.id, table.id, column.physicalName)}>
								{@html column.description ? column.description.replace(/\r\n|\n/g, '<br>') : ''}
							</td>
						</tr>
					{/each}
				{/each}
...
<Dialog open={isOpenDescriptionModal} on:close={toggleDescriptionModal}>
	<div slot="title">비고</div>
	<div class="p-2">
		<TextField autofocus multiline rows="3" cols="32" bind:value={tempDescription} />
	</div>
	<div slot="actions">
		<Button variant="fill" color="primary" on:click={bindValue}>OK</Button>
